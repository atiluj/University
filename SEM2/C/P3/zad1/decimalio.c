...
static bool is_digit(const char c){  //zwraca prawde jeœli c jest z przedzialu 1-9, wpp. fa³sz
    return '0' <= c && c <= '9';
}

static bool is_space_or_newline(const char c){   //zwraca prawde jesli c jest spacj¹ lub znakiem nowej linii, wpp fa³sz
    return c == ' ' || c == '\n';
}
...
int read_decimal(){   //czyta znak
    ...
    do {//pobiera pierwsza liczbe do spacji i zapisuje w d
        d *= 10;
        d += c - '0'; //chcem aby d bylo int. '0'-48 '1'-49 ... '9'-57, wiêc przykladowo 57-48=9 i mamy int.
        c = getchar();
    }while( is_digit(c) );
