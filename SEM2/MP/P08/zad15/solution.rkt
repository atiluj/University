...
(struct thunk (e env) #:transparent) ; thunk bez spamiętywania <----------------- ZAD15

;;Funkcja value? się nie zmienia, bo nie chcemy zwracać niepoliczonych thunków jako wartości, tylko je wyliczać.
...
(define (eval-env e env)
  (match e
    ...
    [(var-expr x) (match (env-lookup x env)
                    [(thunk e1 env) (eval-env e1 env)] ;; <----------------- ZAD15
                    [_ (env-lookup x env)])]
    [(if-expr eb et ef) (if (eval-env eb env)
                            (eval-env et env)
                            (eval-env ef env))]
    [(cons-expr e1 e2) (cons (thunk e1 env)  ;; <----------------- ZAD15
                             (thunk e2 env))];; <----------------- ZAD15
    ...
    [(lam x e) (clo x e env)] ; <--------------------------------- !!!
    [(app f e) ; <------------------------------------------------ !!!
     (let ([vf (eval-env f env)] ;; <----------------- ZAD15
           [ve (thunk e env)]) ;;
       (match vf [(clo x body fun-env) ;;
                  (eval-env body (env-add x ve fun-env))]))] ;;
    [(thunk e env-thu) (eval-env e env-thu)])) ;;
